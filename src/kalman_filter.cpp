...
void KalmanFilter::NormalizeAngle(double& phi){
  //Normalizing phi between -pi and pi
...
void KalmanFilter::Update(const VectorXd &z) {
  /**
  TODO:
    DONE: * Updating the state by using Kalman Filter equations
  */
  
  VectorXd z_pred = H_ * x_;
  VectorXd y = z - z_pred;
  
  //Calling Estimator function to complete the update
  Estimator(y);
}
...
void KalmanFilter::UpdateEKF(const VectorXd &z) {
  /**
  TODO:
    DONE: * Updating the state by using Extended Kalman Filter equations
  */
  ...
  //Precalculating reused quantities for error calculation
  ...
  //Adjusting phi value to be between -pi and pi.
  NormalizeAngle(y(1));
